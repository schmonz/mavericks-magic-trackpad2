 ...
 * device-specific h->ctx — so they are identical for any device. A reader's gh_adapter_t fills these
 * six slots with the defaults and supplies only its three genuinely-specific callbacks: init_attach
 * (build the seeded dict + attach), interpose (install its seam), restore (remove its seam).
 */
