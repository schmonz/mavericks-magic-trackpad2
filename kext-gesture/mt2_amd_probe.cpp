...
static void probe_run(int n) {
    ...
    if (n == 1) {
        ...
        IOLog("mt2_amd_probe: CHURN n=%d final amd_count=%d base=%d -> %s\n",
              n, fin, base, (fin == base) ? "PASS" : "LEAK");
    }
}

static int mt2_amd_probe_sysctl SYSCTL_HANDLER_ARGS {
...
SYSCTL_PROC(_debug, OID_AUTO, mt2_amd_probe, CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED,
            0, 0, mt2_amd_probe_sysctl, "I",
            "MT2 fabricated-AMD oracle: 1=build, 0=teardown, N>=2=churn N cycles");

static bool gRegistered = false;
void mt2_amd_probe_register(void)   { if (!gRegistered) { sysctl_register_oid(&sysctl__debug_mt2_amd_probe); gRegistered = true; } }
void mt2_amd_probe_unregister(void) { if (gRegistered) { sysctl_unregister_oid(&sysctl__debug_mt2_amd_probe); gRegistered = false; } }
