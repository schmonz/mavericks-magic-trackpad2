 ...
 * (mavericks_splice_kext.h -> ../src/mavericks_splice.h -> vtable_clone.h, which #errors without them). */
#define VTC_ALLOC(sz)  IOMalloc(sz)
#define VTC_FREE(p,sz) IOFree((p), (sz))
#include "mavericks_splice_kext.h"
...
static int ks_clone_override(void *ctx, void *t, const mavericks_splice_row_t *r, mavericks_splice_state_t *st) {
    (void)ctx;
...
static void ks_clone_restore(void *ctx, void *t, mavericks_splice_state_t *st) {
    (void)ctx; vtc_restore(t, &st->clone);
}

const mavericks_splice_ops_t mavericks_splice_kext_ops = {
