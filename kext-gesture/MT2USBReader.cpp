...
#include <libkern/c++/OSMetaClass.h>    /* allocClassWithName (inlined manual-start callbacks below) */
#include <libkern/c++/OSDictionary.h>
#include <string.h>                     /* strcmp (inlined gh_default_class_ok); genuine_host.h via the .h */
...
/* Manual-start adapter callbacks — INLINED here (formerly the shared gh_default_adapter, dropped since
 * USB is now the only genuine_host client). They use ONLY h->cfg (class names) + h->obj/h->provider —
 * never device-specific ctx — so they are trivial glue over the genuine_host engine. USB supplies only
 * the two genuinely-specific ops: interpose (install the handleReport seam) + restore. */
static void *gh_default_alloc(gh_host_t *h) {
    OSObject *o = OSMetaClass::allocClassWithName(h->cfg->driver_class);
    IOService *s = OSDynamicCast(IOService, o);
    if (!s && o) o->release();
    return s;
}
static bool gh_default_class_ok(gh_host_t *h) {
    const char *cls = ((IOService *)h->obj)->getMetaClass()->getClassName();
    return cls && strcmp(cls, h->cfg->safety_class) == 0;
}
static bool gh_default_init_attach(gh_host_t *h) {
...
static bool gh_default_start(gh_host_t *h)     { return ((IOService *)h->obj)->start((IOService *)h->provider); }
static void gh_default_detach(gh_host_t *h)    { ((IOService *)h->obj)->detach((IOService *)h->provider); }
static void gh_default_terminate(gh_host_t *h) { ((IOService *)h->obj)->terminate(); }
static void gh_default_release(gh_host_t *h)   { ((IOService *)h->obj)->release(); }
