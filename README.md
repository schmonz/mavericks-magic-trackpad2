...
1. **`MT2USBClaim`** (input kext) out-bids the generic kernel HID driver for the
...
kexts at startup from a loose path and re-enumerates the device if `MT2USBClaim`
...
## Uninstall
    ...
    sudo kextunload -b com.schmonz.MT2USBClaim
...
- `kext-usbclaim/` — `MT2USBClaim`, the input interface-claim kernel extension.
