...
typedef struct {
    ...
    int n_feed;  MavericksTouchFrame last_feed; MavericksTouchFrame feeds[8];
    int n_arm;   uint32_t last_arm;
} rec_t;
static void rec_click(void *c, unsigned m){ rec_t*r=c; r->n_click++; r->last_mask=m; }
static void rec_feed (void *c, const MavericksTouchFrame *f){ rec_t*r=c; if(r->n_feed<8) r->feeds[r->n_feed]=*f; r->n_feed++; r->last_feed=*f; }
...
static MavericksTouchFrame one(int x){ MavericksTouchFrame f; memset(&f,0,sizeof f);
...
static void run_tests(void) {
    ...
    { mavericks_session_t s; memset(&s,0,sizeof s); rec_t r={0}; mavericks_session_sink_t k=mk(&r);
      ...
      rec_t t1={0}; k=mk(&t1); mavericks_session_timer(&s,&k);
    ...
    { mavericks_session_t s; memset(&s,0,sizeof s); rec_t r={0}; mavericks_session_sink_t k=mk(&r);
      ...
      mavericks_session_frame(&s, BT, &f, 5005, &k);
    ...
    { mavericks_session_t s; memset(&s,0,sizeof s); rec_t r={0}; mavericks_session_sink_t k=mk(&r);
      ...
      mavericks_session_frame(&s, USB, &f, 5005, &k);
