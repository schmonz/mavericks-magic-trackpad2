...
static int fm_clone_override(void *ctx, void *t, const mavericks_splice_row_t *r, mavericks_splice_state_t *st){
...
static int fm_clone_override_fail(void *ctx, void *t, const mavericks_splice_row_t *r, mavericks_splice_state_t *st){
    (void)ctx;(void)t;(void)r;(void)st; return -1;
}
static void fm_clone_restore(void *ctx, void *t, mavericks_splice_state_t *st){
    (void)t; fakemem_t *m = (fakemem_t*)ctx;
...
static mavericks_splice_ops_t mk_ops(fakemem_t *m){
    mavericks_splice_ops_t o; o.read_slot=fm_read; o.write_slot=fm_write; o.class_name=fm_class;
...
static void run_tests(void) {
    ...
    { fakemem_t m; memset(&m,0,sizeof m);
      ...
      mavericks_splice_row_t row = {"mem", MAVERICKS_SPLICE_MEM_SLOT, MAVERICKS_GATE_SLOT_POPULATED, 0,0, S, (void*)shimA, 0,0,0};
      mavericks_splice_state_t st; memset(&st,0,sizeof st);
      mavericks_splice_ops_t ops = mk_ops(&m);
      CHECK_EQ(mavericks_splice_install(&row,(void*)1,&ops,&st), MAVERICKS_SPLICE_OK);
      ...
      CHECK(st.installed);
      CHECK_EQ(mavericks_splice_install(&row,(void*)1,&ops,&st), MAVERICKS_SPLICE_ALREADY);
      mavericks_splice_restore((void*)1,&row,&ops,&st);
    ...
    { fakemem_t m; memset(&m,0,sizeof m);
      ...
      mavericks_splice_state_t st; memset(&st,0,sizeof st);
      mavericks_splice_ops_t ops = mk_ops(&m);
      mavericks_splice_install(&row,(void*)1,&ops,&st);
      mavericks_splice_restore((void*)1,&row,&ops,&st);        /* slot back to origcb */
    ...
    { fakemem_t m; memset(&m,0,sizeof m);
      ...
      mavericks_splice_state_t st; memset(&st,0,sizeof st);
      mavericks_splice_ops_t ops = mk_ops(&m);
      CHECK_EQ(mavericks_splice_install(&row,(void*)1,&ops,&st), MAVERICKS_SPLICE_NOT_READY);
    ...
    { fakemem_t m; memset(&m,0,sizeof m);
      ...
      mavericks_splice_row_t row = {"clone", MAVERICKS_SPLICE_VTABLE_CLONE, MAVERICKS_GATE_NONE, 0,0,
    ...
    { fakemem_t m; memset(&m,0,sizeof m); m.cls = "SomethingElse";
      mavericks_splice_row_t row = {"clone", MAVERICKS_SPLICE_VTABLE_CLONE, MAVERICKS_GATE_CLASS_NAME,
                              "BNBTrackpadDevice", "BluetoothMultitouchTransport",
                              3, (void*)shimA, 0,0, 0x2000};
      mavericks_splice_state_t st; memset(&st,0,sizeof st);
      mavericks_splice_ops_t ops = mk_ops(&m);
      CHECK_EQ(mavericks_splice_install(&row,(void*)1,&ops,&st), MAVERICKS_SPLICE_ABORT);
    ...
    { fakemem_t m; memset(&m,0,sizeof m);
      mavericks_splice_row_t row = {"clone", MAVERICKS_SPLICE_VTABLE_CLONE, MAVERICKS_GATE_NONE, 0,0,
                              3, (void*)shimA, 0,0, 0x2000};
      mavericks_splice_state_t st; memset(&st,0,sizeof st);
      mavericks_splice_ops_t ops = mk_ops(&m); ops.clone_override = fm_clone_override_fail;
      CHECK_EQ(mavericks_splice_install(&row,(void*)1,&ops,&st), MAVERICKS_SPLICE_OP_FAIL);
      CHECK(!st.installed);
      mavericks_splice_restore((void*)1,&row,&ops,&st); }
}
TEST_MAIN()
