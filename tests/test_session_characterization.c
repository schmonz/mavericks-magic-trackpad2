/* Characterization net for the mavericks_session SECOND output channel and control paths that the
...
static void rec_feed (void *c, const MavericksTouchFrame *f)     { ev_t e={0}; e.kind=EV_FEED; e.contact_count=f->contact_count; put(c,e); }
static void rec_arm  (void *c, uint32_t ms)                   { ev_t e={0}; e.kind=EV_ARM; e.ms=ms; put(c,e); }
static mavericks_session_sink_t mk(rec_t *r){
    mavericks_session_sink_t s; s.post_button_edge=rec_click; s.feed_frame=rec_feed; s.arm_timer=rec_arm; s.ctx=r; return s;
}
...
static MavericksTouchFrame contact(int x, int pressure, int button) {
...
static void s1_button_edge(void) {
    ...
    MavericksTouchFrame f2 = contact(50, 20, 1);  mavericks_session_frame(&s, USB, &f2, 5010, &k);
    MavericksTouchFrame f3 = contact(50, 20, 0);  mavericks_session_frame(&s, USB, &f3, 5015, &k);
...
static void s5_full_lift_absence_pump(void) {
    ...
    MavericksTouchFrame lift = contact(70, 0, 0);       /* same contact, pressure 0 */
    mavericks_session_frame(&s, BT, &lift, 5010, &k);
...
static void s6_timer_watchdog_flush(void) {
    ...
    r.n = 0;
    mavericks_session_timer(&s, &k);                       /* nothing left after flush */
    CHECK_EQ(r.n, 0);
}
