...
static void run_tests(void) {
    ...
    { mt2_lifecycle_t lc; mt2_lifecycle_reset(&lc);
      int ids[] = {3};
      MavericksTouchFrame f1 = frame_of(1, ids); mt2_lifecycle_step(&lc, &f1);
      MavericksTouchFrame f2 = frame_of(1, ids); mt2_lifecycle_step(&lc, &f2);
    ...
    { mt2_lifecycle_t lc; mt2_lifecycle_reset(&lc);
      int ids2[] = {1, 4}; MavericksTouchFrame f1 = frame_of(2, ids2); mt2_lifecycle_step(&lc, &f1);
      int ids1[] = {1};    MavericksTouchFrame f2 = frame_of(1, ids1); mt2_lifecycle_step(&lc, &f2);
    ...
    { mt2_lifecycle_t lc; mt2_lifecycle_reset(&lc);
      int ids1[] = {1};        MavericksTouchFrame f1 = frame_of(1, ids1); mt2_lifecycle_step(&lc, &f1);
      int ids2[] = {1, 4};     MavericksTouchFrame f2 = frame_of(2, ids2); mt2_lifecycle_step(&lc, &f2);
      int s1 = slot_of(&f2, 1), s4 = slot_of(&f2, 4);
      CHECK_EQ(f2.transducers[s1].state, TS_TOUCHING);
    ...
    { mt2_lifecycle_t lc; mt2_lifecycle_reset(&lc);
      int ids[] = {3};
      MavericksTouchFrame f1 = frame_of(1, ids); mt2_lifecycle_step(&lc, &f1);
      MavericksTouchFrame gone; memset(&gone, 0, sizeof gone); mt2_lifecycle_step(&lc, &gone);
      MavericksTouchFrame f2 = frame_of(1, ids); mt2_lifecycle_step(&lc, &f2);
    ...
    { mt2_lifecycle_t lc; mt2_lifecycle_reset(&lc);
      int ids2[] = {2, 5}; MavericksTouchFrame f1 = frame_of(2, ids2); mt2_lifecycle_step(&lc, &f1);
      MavericksTouchFrame out; int any = mt2_lifecycle_flush(&lc, &out);
      CHECK_EQ(any, 1);
      ...
      MavericksTouchFrame out2; CHECK_EQ(mt2_lifecycle_flush(&lc, &out2), 0); }

    /* flush with nothing active returns 0. */
    { mt2_lifecycle_t lc; mt2_lifecycle_reset(&lc);
      MavericksTouchFrame out; CHECK_EQ(mt2_lifecycle_flush(&lc, &out), 0); }

    /* reset() forgets history: no pending end, next contact is new. */
    { mt2_lifecycle_t lc; mt2_lifecycle_reset(&lc);
      int ids[] = {2}; MavericksTouchFrame f1 = frame_of(1, ids); mt2_lifecycle_step(&lc, &f1);
      mt2_lifecycle_reset(&lc);
      MavericksTouchFrame out; CHECK_EQ(mt2_lifecycle_flush(&lc, &out), 0);   /* nothing pending */
      MavericksTouchFrame f2 = frame_of(1, ids); mt2_lifecycle_step(&lc, &f2);
      CHECK_EQ(f2.transducers[0].state, TS_START); }
}
TEST_MAIN()
