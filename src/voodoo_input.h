...
#define MAX_TOUCHES VOODOO_INPUT_MAX_TRANSDUCERS   /* legacy alias, same value */
...
typedef struct {
    ...
    VoodooInputTouch currentCoordinates;
    VoodooInputTouch previousCoordinates;   /* unread today, zero-initialized */

    /* --- beyond VoodooInput: consumed by mt1_encode, do NOT drop --- */
    touch_state_t state;
    int touch_major, touch_minor;
    int orientation;
} VoodooInputTransducer;
