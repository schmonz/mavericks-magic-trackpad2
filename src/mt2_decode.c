...
int mt2_decode(const uint8_t *report, size_t len, uint8_t report_id,
               size_t header_len, MavericksTouchFrame *frame) {
    ...
    frame->isPhysicalButtonDown = report[1] & 0x01;
    frame->timestamp = 0;
    ...
    for (int i = 0; i < npoints; i++) {
        ...
        MavericksTouchContact *out = &frame->transducers[frame->contact_count++];
        out->id          = t[8] & 0x0f;
        out->state       = (statebits == 0x80) ? TS_TOUCHING : TS_END;
        out->currentCoordinates.x           = x;
        out->currentCoordinates.y           = y;
        out->touch_major = t[4];
        out->touch_minor = t[5];
        out->currentCoordinates.pressure        = t[6];
