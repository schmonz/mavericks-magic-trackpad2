 ...
 * But mavericks_splice.h -> vtable_clone.h #errors unless VTC_ALLOC/FREE exist, and its static
...
#endif
#include "mavericks_splice.h"
#include <string.h>

static int gate_ok(const mavericks_splice_row_t *row, void *target,
                   const mavericks_splice_ops_t *ops, int *out_code) {
    switch (row->gate) {
    case MAVERICKS_GATE_NONE:
        return 1;
    case MAVERICKS_GATE_SLOT_POPULATED: {
        void *cur = ops->read_slot(ops->ctx, target, row->slot);
        if (!cur || cur == row->shim) { *out_code = MAVERICKS_SPLICE_NOT_READY; return 0; }
        return 1;
    }
    case MAVERICKS_GATE_CLASS_NAME: {
        const char *c = ops->class_name(ops->ctx, target);
        if (c && (strcmp(c, row->gate_class) == 0 ||
                  (row->gate_class2 && strcmp(c, row->gate_class2) == 0)))
            return 1;
        *out_code = MAVERICKS_SPLICE_ABORT; return 0;
    }
    }
    *out_code = MAVERICKS_SPLICE_ABORT; return 0;
}
...
int mavericks_splice_install(const mavericks_splice_row_t *row, void *target,
    ...
    if (st->installed) return MAVERICKS_SPLICE_ALREADY;

    int code = MAVERICKS_SPLICE_OK;
    if (!gate_ok(row, target, ops, &code)) return code;

    if (row->kind == MAVERICKS_SPLICE_MEM_SLOT) {
         ...
         * see the mavericks_splice_row_t comment. Capture BEFORE the write. */
    ...
    if (ops->clone_override(ops->ctx, target, row, st) != 0)
        return MAVERICKS_SPLICE_OP_FAIL;
    st->installed = 1;
    return MAVERICKS_SPLICE_OK;
}
